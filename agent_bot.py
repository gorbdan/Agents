...
AGENTS_CONFIG = {
    ...
    "ux": {
        ...
        "focus": (
            "Анализируй с позиции человека который впервые открыл бота и ничего не знает. "
            "Ищи: 1) Онбординг — понятно ли с первого экрана ЧТО делает бот и КАК начать? "
            "2) Пустые состояния — что видит юзер если ничего не сделал, нет аватара, нет промпта? "
            "3) Тупики — ситуации где юзер не понимает что делать дальше (нет подсказки, нет кнопки назад)? "
            "4) Тексты ошибок — понятны ли они обычному человеку или технические? "
            "5) Кнопки — очевидно ли что они делают по названию? "
            "6) Сколько шагов нужно чтобы сделать первую генерацию — это слишком много? "
            "7) Есть ли объяснение что такое изюминки и как их получить? "
            "8) Обратная связь — знает ли юзер что бот получил его запрос и работает?"
        ),
        "keywords": ["reply_text", "send_message", "InlineKeyboard", "ReplyKeyboard",
                     "callback_query", "edit_text", "answer", "BotCommand", "start",
...
UX_FINDING_INSTRUCTIONS = """Ты анализируешь бот глазами человека который открыл его впервые и ничего не знает.

ФОРМАТ КАЖДОЙ НАХОДКИ:

## {emoji} [файл.py:номер_строки] Короткое название

**Сценарий:** какой конкретный пользователь попадает в эту ситуацию

**Что сейчас видит юзер:**
(точный текст сообщения или описание кнопок)

**Проблема:** почему это непонятно новичку

**Предложение:** конкретный новый текст или изменение интерфейса

---

Где {emoji} = 🔴 критично (юзер уходит), 🟡 важно (юзер путается), 🟢 улучшение (было бы лучше).

ПРАВИЛА:
1. Цитируй точный текст из кода (reply_text, send_message и т.д.)
2. Пиши предложение готовым текстом — чтобы можно было скопировать и вставить в код
3. Особое внимание: /start, первая генерация, ошибки баланса, непонятные кнопки
4. Если текст понятен — не репортить
5. Минимум 5 находок, максимум 15
...
def call_agent(agent_key: str) -> str:
    ...
    instructions = UX_FINDING_INSTRUCTIONS if agent_key == "ux" else FINDING_INSTRUCTIONS
    user_msg = (
        "Проанализируй бот глазами новичка. Найди все места где пользователь может запутаться или не понять что делать."
        if agent_key == "ux"
        else ANALYZER_USER_MSG
    )
